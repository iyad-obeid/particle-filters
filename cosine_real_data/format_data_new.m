...
fs_kinematics = 1e3;
fs_neurons = 36e3;
t = (1:length(AD33))/fs_kinematics;
kinematics(1,:) = AD33; % shoulder angle
kinematics(2,:) = AD34; % elbow angle
kinematics(1,:) = kinematics(1,:) + abs(min(kinematics(1,:)));
kinematics(2,:) = kinematics(2,:) + abs(min(kinematics(2,:)));
% kinematics(3,:) = AD35;
% kinematics(4,:) = AD36;
% kinematics(5,:) = AD37;
% kinematics(6,:) = AD38;
...
for k = 1: length(names)
    if strncmp(names(k), 'sig', 3) == 1
        ...
        eval(s);
        firings = false(1,length(t));
        firings(temp) = 1;
        eval(['N', num2str(p), ' = firings;']);
