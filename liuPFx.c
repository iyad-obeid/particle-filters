...
void liu_step4(gM *posn_est, gM *mu_est, gM *posn_sTheta, gM *mu_sTheta, gM *posn_est_new, gM *mu_est_new){
	...
	// best so far
    gsl_matrix_set(posn_est_new,i,0,gsl_matrix_get(posn_est,i,0) + gsl_matrix_get(posn_sTheta,i,0) + gsl_ran_gaussian(r,5));

    for (j=0;j<mu_est->size2;j++)
        for (i=0;i<mu_est->size1;i++)
		// best so far
        gsl_matrix_set(mu_est_new,i,j, gsl_matrix_get(mu_est,i,j) + gsl_matrix_get(mu_sTheta,i,j) + gsl_ran_gaussian(r,0.005));
}
...
filenum = 9501;
    ...
    sprintf(fNameOut,"%sd%05iout1.bin",outDir,filenum);
    if (SIM7FLAG){
      filenum = dataParam[3];
