...
for p =1:nParticles
    ...
    P_old(p).gamma(1,1) = gamma_init;
    P_old(p).beta(1,1) = beta_init;
end
...
x_estimate.gamma(1, :) = gamma_init;
x_estimate.beta(1, :) = beta_init;
...
for n = 1:nNeurons
    ...
    for k = 2:nIterations_training
        ...
        for p = 1:nParticles
            ...
            flag = true;
            while flag
                v = P_old(p).gamma(1,1) + sd.gamma1*randn;
                if v > 0
                    P_new(p).gamma(1,1) = v;
                    flag = false;
                end
            ...
            flag = true;
            while flag
                v = P_old(p).beta(1,1)+ sd.beta1*randn;
                if v > P_new(p).gamma(1,1)
                    P_new(p).beta(1,1) = v;
                    flag = false;
                end
            end
            ...
            temp.gamma(p,1) = P_new(p).gamma(1,1);
            temp.beta(p,1) = P_new(p).beta(1,1);
        end
        ...
        for p = 1:nParticles
            ...
            P_new(p).gamma(1,1)=temp.gamma(resampleIndicies(p),1);
            P_new(p).beta(1,1)=temp.beta(resampleIndicies(p),1);
        end
        ...
        for p = 1:nParticles
            ...
            while flag
                ...
                v = P_new(p).gamma(1,1) + sd.gamma2*randn;
                if v > 0
                    P_new(p).gamma(1,1) = v;
                    flag = false;
                end
            ...
            flag = true;
            while flag
                v = P_new(p).beta(1,1) + sd.beta2*randn;
                if v > P_new(p).gamma(1,1)
                    P_new(p).beta(1,1) = v;
                    flag = false;
                end
        ...
        for p = 1:nParticles
            ...
            x_estimate.gamma(k,n) = P_new(p).gamma(1,1)*P_new(p).w(1,1) + x_estimate.gamma(k,n);
            x_estimate.beta(k,n) = P_new(p).beta(1,1)*P_new(p).w(1,1) + x_estimate.beta(k,n);
            x_estimate.s(k,1) = s_ds(k,1);
        end
        P_old = P_new;
    end
    tElapsed = toc/60;
    disp(['Elapsed time = ', num2str(tElapsed), ' minutes.']);
end

theta_p_final = x_estimate.theta_p(end, :);
gamma_final = x_estimate.gamma(end, :);
beta_final = x_estimate.beta(end, :);
s_final = x_estimate.s(end,1);

save training_data_3_params
save initial_estimates k  tTrain blockSize dt ...
    nNeurons neuron_list nIterations_training ...
    theta_p_final gamma_final beta_final s_final

t2 = 0:dt*blockSize:tTrain-dt;
for k = 1:nNeurons
    figure(neuron_list(k)),
    set(gcf,'WindowStyle','docked'),
    subplot(311), plot(t2, x_estimate.theta_p(:,k), 'r'), grid on, ...
        xlim([0 tTrain]), ylim([0 pi]), title('\theta_{p} estimate'),
    subplot(312), plot(t2, x_estimate.gamma(:,k), 'r'), grid on,...
        xlim([0 tTrain]), title('\gamma estimate'),
    subplot(313), plot(t2, x_estimate.beta(:,k), 'r'), grid on,...
        xlim([0 tTrain]), title('\beta estimate');
end